<script lang="ts">
	import Draggable from './draggable.svelte';

	let {
		initialPos,
		pos = $bindable({ x: 0, y: 0 }),
		index,
		zoom = $bindable(1),
		focused = $bindable(),
		createconn,
		pinPos = $bindable(),
		zCounter = $bindable(),
	} = $props<{
		initialPos: { x: number; y: number };
		pos: { x: number; y: number };
		index: number;
		focused: number | undefined;
		zoom: number;
		createconn: (index: number) => void;
		pinPos: { x: number; y: number };
		zCounter: number;
	}>();
	let text = $state('');

	let down = $state(false);
	let doubleclick = $state(false);

	function onblur() {
		doubleclick = false;
	}

	let velX = 0;
	let smoothVelX = 0;
	let prev = { x: 0, t: performance.now() };

	const DELTA_ANGLE = 4;
	let angle = $state(0);

	$effect(() => {
		if (!down) {
			return;
		}
		const now = performance.now();

		if (!prev) {
			prev = { x: pos.x, t: now };
			return;
		}

		const dt = now - prev.t;

		if (dt > 5) {
			velX = (pos.x - prev.x) / dt;
			smoothVelX = 0.8 * smoothVelX + 0.2 * velX;
			angle = smoothVelX * DELTA_ANGLE;
		}

		prev = { x: pos.x, t: now };
	});
</script>

<Draggable
	{createconn}
	initialPos={{ x: initialPos.x, y: initialPos.y, w: 200, h: 200 }}
	bind:pinPos
	bind:pos
	bind:down
	bind:doubleclick
	bind:zoom
	bind:focused
	bind:zCounter
	{index}
	type="note"
	aspectRatio={1}
>
	<div
		role="navigation"
		style="transform: rotate({angle}deg)"
		class="pointer-events-none h-full w-full bg-[url(/src/lib/assets/stickynote.png)] drop-shadow-[19px_10px_21px_rgba(0,0,0,0.2)] bg-cover select-none"
	>
		{#if !doubleclick}
			<p
				class="absolute inset-5 resize-none overflow-hidden wrap-break-word whitespace-pre-wrap select-none text-3xl"
			>
				{text}
			</p>
		{:else}
			<textarea
				class="absolute inset-5 resize-none overflow-hidden wrap-break-word whitespace-pre-wrap text-3xl"
				bind:value={text}
				autofocus
				{onblur}
			>
			</textarea>
			<p></p>
		{/if}
	</div>
</Draggable>
